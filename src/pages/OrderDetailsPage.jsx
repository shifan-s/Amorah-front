...
import { cancelMyOrder, downloadOrderInvoice, getOrderByNumber } from '../services/orderService.js';
    ...
    <>
      ...
      <section>
        ...
        <div className="mt-6 border border-amorah-border bg-amorah-light p-4 text-sm leading-6 text-amorah-brown">
          Orders can be cancelled only before dispatch. Once an order has been dispatched, cancellation, return and refund options are not available.
        </div>
        <Modal open={cancelOpen} onClose={() => !actionBusy && setCancelOpen(false)} title="Cancel order">
          <div className="space-y-4"><p>Are you sure you want to cancel this order?</p><p>This action cannot be undone.</p><label className="block font-semibold">Cancellation reason (optional)<textarea value={cancelReason} onChange={(event) => setCancelReason(event.target.value)} maxLength={500} className="mt-1 w-full border border-amorah-border p-3" /></label><div className="flex justify-end gap-3"><button disabled={actionBusy} onClick={() => setCancelOpen(false)} className="min-h-11 border border-amorah-border px-4">Keep Order</button><button disabled={actionBusy} onClick={handleCancel} className="min-h-11 bg-amorah-maroon px-4 font-semibold text-white disabled:opacity-50">{actionBusy ? 'Cancelling…' : 'Cancel Order'}</button></div></div>
        </Modal>
      </section>
    </>
  );
